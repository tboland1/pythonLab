...
def modify(frame, input, output):
	...
	# node.source == input ?? current understanding would indicate this...
	test= input.particle_properties.particle_type
	##print(test.array)
	...
	all_type=input.particle_properties['Particle Type'].marray[:]			# particle type to sort data
	##print("The particle type of all atoms are:", all_type) 					# test if the position array works
	all_pos=input.particle_properties['Position'].marray[:] 				# position of all atoms
	##print("The position of all atoms are:", all_pos) # test if the position array works
	...
	type_bool=output.particle_properties['Particle Type'].marray[:] 		# ids for the manual selection modifier
	##print("The manual selection type boolean is:", type_bool)
	...
	############ get the atom IDs for all the selected particles & separate the ce from the o ############
	selected_ids=[]
	selected_ids_ce=[]
	for i in range(0,len(all_ids)):
		if selection_bool[i] != 0 and type_bool[i] != 1:
			selected_ids.append(all_ids[i])
		elif selection_bool[i] != 0 and type_bool[i] == 1:
			selected_ids_ce.append(all_ids[i])
	print("The ids for the selected O atoms are:",selected_ids,'and the ceriums are', selected_ids_ce,'\n')
	...
	for current_id in selected_ids:
		##print('The current O id is:',current_id)
		# x,y,z position for the maunally selected atoms array
		for match_id,match_pos in zip(all_ids,all_pos):
			if current_id == match_id:
				##print('The current id for o is:',current_id, 'with a match id of', match_id,'and position of', match_pos)
				migration_data['migration_element'+ str(current_id)]={'atomID':current_id,'v_pos':match_pos}
	# for Ce
	for current_id_ce in selected_ids_ce:
		##print('The current Ce id is:',current_id_ce)
		# x,y,z position for the maunally selected atoms array
		for match_id,match_pos in zip(all_ids,all_pos):
			if current_id_ce == match_id:
				##print('The current id for ce is:',current_id_ce, 'with a match id of', match_id,'and position of', match_pos)
				charge_data['migration_element'+ str(current_id_ce)]={'atomID':current_id_ce,'v_pos':match_pos}
	##print('The migration/charge data dict is: \n', migration_data,'\n', charge_data)
	##print('The migration/charge values are:\n,',migration_data.values(),'\n',charge_data.values())
	...
	higher_key_ce=list(charge_data.keys())
	##print('The first level directory keys are:',  higher_key_ce )
	sub_y_key_ce=list((charge_data[higher_key_ce[1]]).keys())
	##print( 'The section level directory keys are:', sub_y_key_ce, '\n')
	...
	# for ce
	list_y_values_ce=[]
	for i in higher_key_ce:
		list_y_values_ce.append(charge_data[i]['v_pos'][1])
	list_y_values_ce=sorted(list_y_values_ce)
	##print('The sorted y values are:', list_y_values_ce)
	...
	# for ce
	charge_order=[]
	for match_y in list_y_values_ce:
		for higher_key_ce in charge_data:
			if charge_data[higher_key_ce]['v_pos'][1] == match_y:
				print(charge_data[higher_key_ce]['v_pos'][1], higher_key_ce)
				charge_order.append(higher_key_ce)
	charge_order=list(reversed(charge_order))
	print('The sorted charge higher keys are:',charge_order)
	...
	for counter in range(0,len(migration_order)-1):
		...
		if counter == 0:
			...
			ce3_1=charge_data[charge_order[counter]]['atomID']
			ce3_2=charge_data[charge_order[counter+1]]['atomID']
			nextcounter=counter+1
			##print("The charge key for the Ce's are:", ce3_1, ce3_2)
		...
		# Create the intital state by moving the atom_prev by the pos_prev the rest of the typical hops
			...
			# make sure you assign the ce3+ at the correct time
			ce3_1=charge_data[charge_order[nextcounter]]['atomID']
			ce3_2=charge_data[charge_order[nextcounter+1]]['atomID']
			nextcounter+=1
			print(counter,nextcounter)
			temp		=[]
		...
		if counter == 0:
			...
			with open(fileopen_i,'w+') as f:
				...
				f.write('{}\n'.format(' '))
				f.write('{}{}{}{}\n'.format('group ce3p id ',ce3_1,' ',ce3_2) )
				f.write('{}\n'.format('set group ce3p type 3') )
				f.write('{}\n'.format('set type 3 charge 3') )
				f.close()
		...
		# writing the typical initial state & trans state include files
			...
			with open(fileopen_i,'w+') as f:
				...
				f.write('{}{}{}{}\n'.format('group ce3p id ',ce3_1,' ',ce3_2) )
				f.write('{}{}{}{}\n'.format('group ce4p id ',ce4_1,' ',ce4_2) )
				f.write('{}\n'.format('set group ce3p type 3') )
				f.write('{}\n'.format('set group ce4p type 4') )
				f.write('{}\n'.format('set type 3 charge 3') )
				f.write('{}\n'.format('set type 4 charge 4') )
				f.close()
			# writing the transition state
			with open(fileopen_t,'w+') as f:
