...
def modify(frame, input, output):
	...
	for i in range(0,len(all_ids)):
		if selection_bool[i] != 0 and type_bool[i] != 1:
			...
			selected_ce.append(all_ids[i])
	print("The ids for the selected O atoms are:",selected_ids,'and the ceriums are', selected_ce,'\n')
	...
	for current_id in selected_ids:
		for match_id,match_pos,match_type in zip(all_ids,all_pos,all_type):
			# x,y,z position for the maunally selected atoms array
			if current_id == match_id:
				print(match_id, match_pos,match_type)
				migration_data['migration_element'+ str(current_id)]={'atomID':current_id,'v_pos':match_pos,'atomTYPE':match_type}
