...
def modify(frame, input, output):
	...
	for counter in range(0,len(migration_order)-1):
		fileopen='/home/tboland1/migration_hop_'+str(migration_hop)+'.dat'
		if migration_hop%2 != 0:
			mig_key=migration_order[counter-1]
			mig_keyp=migration_order[counter]
		...
		if migration_hop == 0:
			with open(fileopen,'w+') as f:
				atom_ID=migration_data[migration_order[counter]]['atomID']
		...
		# transition state: if the file hop number is 1, 3, 5 ect the mod will not be 0
		elif migration_hop >= 1 and (migration_hop%2) != 0:
			with open(fileopen,'w+') as f:
				f.write('{}{}\n'.format('group hop id ', atom_ID))
				f.write('{}{}{}\n'.format('displace_atoms hop move ', atom_pos,' units box'))
				f.close()
		# final state stay the same
		elif migration_hop > 0 and (migration_hop%2) != 0:
			with open(fileopen,'w+') as f:
				f.write('{}{}\n'.format('group hop id ', atom_ID))
				f.write('{}{}{}\n'.format('displace_atoms hop move ', atom_pos,' units box'))
