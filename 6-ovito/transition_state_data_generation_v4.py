...
def modify(frame, input, output):
	...
	for counter in range(0,len(migration_order)-1):
		...
		if counter == 0:
			...
			print('Initial migration hop file name is: 								',migration_hop, fileopen_i,'\n')
		# Create the intital state by moving the atom_prev by the pos_prev the rest of the typical hops
			...
			atom_ID_new	=migration_data[mig_key_p]['atomID']
			atom_ID		=migration_data[mig_key]['atomID']
			temp		=[]
			for x1,x2 in zip(migration_data[mig_key]['v_pos'],migration_data[mig_key_p]['v_pos']):
				##print( (x1-x2)/2 )
				temp.append((x1-x2)/2)
			atom_pos	=(' '.join([str(x) for x in temp]))
			##print(' '.join([str(x) for x in temp]))
			print('The intial atom displacement elemets are:				',atom_pos_prev)
			print('The new migrating atom id is currently: 				',atom_ID_new)
			print('The migration key for the 1st ELIF statement is:		', mig_key)
			print('The migration key for the hoping atom is:				', mig_key_p)
			print('Initial migration hop file number:						',migration_hop, fileopen_i,'\n')
		###############################
		# create the transition state
		###############################
		if counter == 0:
			atom_ID		=migration_data[mig_key_t]['atomID']
			temp		=[]
			...
			atom_pos	=(' '.join([str(x) for x in temp]))
			##print(' '.join([str(x) for x in temp]))
			print('The atom displacement elemets are:',atom_pos,'for the atom',atom_ID)
			...
			atom_pos=(' '.join([str(x) for x in temp]))
			##print(' '.join([str(x) for x in temp]))
			print('The migrating atom id is currently:',atom_ID)
			print('The atom displacement elemets are:',atom_pos)
			print('The migrating atom key for the SECOND if statement is:', mig_key)
			print('Migration hop file number:',migration_hop, fileopen_t,'\n')
		###############################
		#  Write out the include migration hop files 
		###############################
		# writing the odd start for the initial state
		if counter == 0:
