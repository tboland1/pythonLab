...
# in tex
# 1NN step edge
#prefix="/home/tboland1/Dropbox/Crozier Group User- Tara Boland/2-lammps/7-step/2-partial-reduce/1-4atomic-layers/4-nn-data/min-1/"
#filecount=['64']
#filename=['min-1NN-']
...
prefix="/home/tboland1/Dropbox/Crozier Group User- Tara Boland/2-lammps/7-step/0-oxidize110/1-4-atomic-layers/4-nn-data/min-1/"
filecount=['57','57']
filename=['min-1NN-','min-2NN-']
## prefix to save the plot
maxxvalue=0.5      # max x value for the axis if o atoms are displaced by an unusally large amount
...
fileprefix=prefix+"/"+ceID+"_"+surfaceID+"_"+vacNid

# The plot title's
title1NN="Step Edge Vacancy 1NN Displacement on (111) Step-Edge"
title2NN="Vacancy 2NN Displacement on (111) Step-Edge"
#----------------------------------------------------------------
...
for file in filelist:
    ...
    if counterfile == 0:
        ...
        y1=o
        x=[x1,x2,y1]
        labels=[r'Ce$^{\rm 3+}$ ('+str(len(ce3))+')',r'Ce$^{\rm 3+}$ ('+str(len(ce4))+')',r'O$^{\rm 2-}$  ('+str(len(o))  +')']
        colors=['cyan','blue','lightgray']
        bins=np.linspace(0, maxxvalue, 40)       
...
#============================== PLOT HISTOGRAM: 2NN =============================================
        ...
        bins=np.linspace(0, maxxvalue, 40)       
        fig = plt.figure()
        plt.hist(x,bins,label=labels,color=colors,rwidth=0.75,linewidth=.4,edgecolor='black',stacked=True)
        plt.margins(x=0.1, y=0.1)
        plt.ylim(0,6+.6)
        plt.legend(loc='upper right')
        ax = plt.axes()
        ...
        fig.savefig(file, transparent=True,figsize=(40,40))  
        ##============================ Ce PLOT DATA: 2NN 
        x2=ce4
        bins=np.linspace(0, maxxvalue , 40)        
        
        fig = plt.figure()
        plt.hist(x2,bins,label=r'Ce$^{\rm 4+}$ ('+str(numce4)+')', edgecolor="black", linewidth=0.4, color='blue', rwidth=.75,stacked=True)
        plt.margins(x=0.1, y=0.1)
        plt.legend(loc='upper right')
        ax = plt.axes()
        ax.yaxis.set_major_locator(MaxNLocator(integer=True))   
        ...
        fig = plt.figure()
        plt.hist(y2,bins,label=r'O$^{\rm 2-}$ ('+str(numo)+')', edgecolor="black", linewidth=0.4, color='lightgray', rwidth=.75,stacked=True)
        plt.margins(x=0.1, y=0.1)
